...
{
    public class AStarWeighted : Algorithm
    {
        private Heap<Node> _openList;
        private Heap<Node> _closedList;

        public AStarWeighted(Grid grid, bool canMoveDiagionally) : base(grid, canMoveDiagionally)
        {
        ...
        protected override List<Node> FindPath()
        {
            while (_openList.Count > 0)
            {
                ...
                foreach (var neighbour in neighbours)
                {
                    ...
                    var newCost = node.GCost + node.Weight * GetDistance(node, neighbour);
